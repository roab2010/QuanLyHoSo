...
import { LayoutDashboard, FolderOpen, Settings, Users, Box, UsersRound, FileText, FileSpreadsheet, CheckSquare, LogOut, Camera, User, ShieldAlert, GitMerge } from 'lucide-react';
import api from "./api";
...
export default function Sidebar({ admin, activeNav, setActiveNav, NAV_ITEMS, onShowModal, onLogout }) {
    ...
    const getVisibleNavItems = () => {
        return NAV_ITEMS.filter(label => {
            ...
            if (label === "Quản lý quy trình") return admin?.role === 'admin';
                                    ...
                                    label === "Danh mục dự án" ? <Settings size={20} color="#9ca3af" /> :
                                        label === "Quản lý tài liệu" ? <FileSpreadsheet size={20} color="#3b82f6" /> :
                                            label === "Duyệt tài liệu" ? <CheckSquare size={20} color="#8b5cf6" /> :
                                                label === "Quản lý quy trình" ? <GitMerge size={20} color="#0284c7" /> :
