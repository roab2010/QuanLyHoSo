                                            ...
                                            <div className="side-doc-list">
                                                {(selectedProject.documents || []).length > 0 ? (
                                                    selectedProject.documents.map(doc => {
                                                        ...
                                                        return (
                                                            <div key={doc.id} className={`side-doc-item ${isPendingSlot ? 'slot-required' : ''}`} onClick={() => {
                                                                if (isPendingSlot) return;
                                                                if (isImage(doc.file_url)) {
                                                                    setPreviewingImage(doc.file_url);
                                                                } else {
                                                                ...
                                                                <div className="side-doc-icon-wrapper" style={{ position: 'relative' }}>
                                                                    <span className="material-symbols-outlined" style={{ color: isPendingSlot ? '#dc2626' : 'var(--primary)' }}>
                                                                        {isPendingSlot ? 'cloud_upload' : 'description'}
                                                                    </span>
                                                                    {isPendingSlot && (
                                                                        <input 
                                                                            type="file" 
                                                                            style={{ position: 'absolute', inset: 0, opacity: 0, cursor: 'pointer' }}
                                                                            onChange={(e) => handleProjectFileUpload(selectedProject.id, e.target.files[0], doc.id)}
                                                                        />
                                                                    )}
                                                                </div>
                                                                <div className="doc-meta">
                                                                    <strong title={doc.document_name} style={{ color: isPendingSlot ? '#dc2626' : 'inherit' }}>
                                                                        {doc.document_name}
                                                                        {isPendingSlot && ' (*)'}
                                                                    </strong>
                                                                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginTop: '4px' }}>
                                                                        <span className="doc-st-pill" style={{ background: docColors.bg, color: docColors.color }}>{docLabel}</span>
                                                                        <small>{isPendingSlot ? 'Chưa nộp' : doc.uploaded_at}</small>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        );
